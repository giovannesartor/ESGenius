...
export function OnboardingWizard({ token, onComplete }: OnboardingWizardProps) {
  ...
  const handleCreateCompany = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("onboarding.failedCreateCompany"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-8">
          ...
          {step === 0 && (
            <div>
              <div className="text-center mb-6">
                ...
                <h2 className="text-xl font-bold text-foreground">{t("onboarding.welcomeTitle")}</h2>
                <p className="text-sm text-muted-foreground mt-2">{t("onboarding.welcomeSubtitle")}</p>
              </div>
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label>{t("onboarding.companyNameLabel")}</Label>
                  <Input placeholder={t("onboarding.companyNamePlaceholder")} value={company.name} onChange={(e) => setCompany({ ...company, name: e.target.value })} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label>{t("onboarding.sectorLabel")}</Label>
                    <Input placeholder={t("onboarding.sectorPlaceholder")} value={company.sector} onChange={(e) => setCompany({ ...company, sector: e.target.value })} />
                  </div>
                  <div className="space-y-1.5">
                    <Label>{t("onboarding.countryLabel")}</Label>
                    <Input placeholder={t("onboarding.countryPlaceholder")} value={company.country} onChange={(e) => setCompany({ ...company, country: e.target.value })} />
                  </div>
                </div>
          ...
          {step === 1 && (
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-foreground">{t("onboarding.uploadTitle")}</h2>
              <p className="text-sm text-muted-foreground mt-2 mb-8">
                {t("onboarding.uploadSubtitle")}
              </p>
